                ...
                using (DataContext dataContext = GetDataContext())
                {
                    ...
                    foreach (string currentFileName in scriptsToRun)
                    {
                        ...
                        using (TransactionScope ts = dataContext.StartTransaction(TransactionScopeOption.Required))
                        {
                            ...
                            Log.Logger.Information("Validating current script: {0}", currentFileName);
                            bool validateFileResult = _database.ValidateFile(currentFileName);
                            if (ValidateFiles && !validateFileResult)
                            {
                                throw new SystemException($"Error validating script {currentFileName}");
                            }
