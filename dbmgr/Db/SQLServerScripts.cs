...
namespace dbmgr.utilities.db
{
    public class SQLServerScripts : IDBScripts
    {
        ...
        {
            return string.Format(@"select o.name, 'IF EXISTS ( SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'''+o.name+''')) BEGIN'+CHAR(10)+'DROP '+ CASE WHEN o.type = 'P' THEN 'PROCEDURE' WHEN o.type = 'V' THEN 'VIEW' WHEN o.type = 'TR' THEN 'TRIGGER' WHEN o.type = 'SN' THEN 'SYNONYM' WHEN o.type in ('FN', 'TF', 'AF','IF') THEN 'FUNCTION' ELSE '<object type not supported>' END +' [' + o.name + '] END' + CHAR(10) + 'GO' + CHAR(10) + m.definition from sys.objects o inner join sys.sql_modules m on o.object_id = m.object_id where o.is_ms_shipped = 0 AND o.type in ({0})
